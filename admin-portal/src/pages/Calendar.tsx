...
export default function FacilityCalendar() {
  ...
  const getPeriodsForFacility = (facility: any): TimelinePeriod[] => {
    ...
    if (facility.bookings && facility.bookings.length > 0) {
      facility.bookings.forEach((item: any) => {
        // Handle nested booking object for ROOMS (RoomOnBooking -> RoomBooking)
        // For others, the item itself is the booking
        const booking = selectedFacilityType === "ROOMS" ? item.booking : item;
        ...
        if (selectedFacilityType === "ROOMS") {
          ...
          // Photoshoots have specific times, but for calendar view we might just show the date block
          // or if it spans midnight, handle that. Usually single day.
          startDate = new Date(booking.startTime || booking.bookingDate);
          endDate = new Date(booking.endTime || booking.bookingDate);
        } else {
          // HALLS and LAWNS
          ...
          if (booking.endDate) {
            ...
            endDate = addDays(startDate, booking.numberOfDays - 1);
          } else {
            ...
            <div className="flex items-center gap-2 w-full lg:w-auto justify-between lg:justify-normal">
              ...
              <div className="flex items-center gap-2">
                {viewMode === 'MONTH' ? (
                  ...
                  <div className="font-semibold min-w-[200px] text-center text-sm sm:text-base border p-2 rounded-md">
                      ...
                      <div key={facility.id} className="flex border-b hover:bg-muted/30 transition-colors">
                        ...
                        <div className="w-48 flex-shrink-0 border-r p-3 font-medium text-sm flex items-center">
                          <div className="d-flex flex-col overflow-hidden">
                            ...
                            {(selectedFacilityType === "HALLS" || selectedFacilityType === "LAWNS") && (
                              <div className="text-xs text-muted-foreground truncate flex items-center gap-1">
                                <Users className="h-3 w-3" />
                                Cap: {facility.capacity || facility.maxGuests || 0}
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="relative flex-1" style={{ minHeight: '80px' }}>
                          ...
                          {/* Period bars */}
                          <div className="absolute inset-0 flex items-start justify-center px-1 py-1.5">
                                  ...
                                  <TooltipProvider key={period.id}>
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <div
                                          className={cn(
                                            "absolute h-5 rounded cursor-pointer transition-all",
                                            getPeriodColor(period.type, period.data.paymentStatus)
                                          )}
                                          style={{
                                            ...style,
                                            top: period.type === 'booking' ? '4px' :
                                              period.type === 'reservation' ? '28px' : '52px'
                                          }}
                                          onClick={() => setSelectedPeriod({ period, facility })}
                                        >
                                          <div className="text-white text-[10px] px-1 truncate leading-5">
                                            {period.type === 'booking' && (period.data.member?.Name || period.data.guestName || period.data.memberName)}
                                      ...
                                      <TooltipContent className="max-w-xs text-xs">
                                        <div className="space-y-1">
                                          ...
                                          {period.type === 'booking' && (
                                            <>
                                              <div>Guest: {period.data.member?.Name || period.data.guestName || period.data.memberName}</div>
                                              {selectedFacilityType === "ROOMS" && (
                                                <div>
                  ...
                  {selectedPeriod.period.type === 'booking' && (
                    <>
                      <div>
                        ...
                        <p className="text-base">{selectedPeriod.period.data.member?.Name || selectedPeriod.period.data.guestName || selectedPeriod.period.data.memberName}</p>
                      </div>
